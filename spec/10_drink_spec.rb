...
describe Drink do
  describe '#initialize' do
    context 'when type is specified and ounces is default' do
      ...
      # beverage type.
      subject(:my_drink) { described_class.new('ranja') }

      # remove the 'x' before running this test
      it 'is your choice of beverage' do
        expect(my_drink.type).to eq('ranja')
      end
  ...
  describe '#full?' do
    context 'when drink has 16 ounces or more' do
      # Create an explicit subject, using 'described_class' and your choice of
      # beverage type.
      subject(:overfull_drink) { described_class.new('ranja') }

      # remove the 'x' before running this test
      it 'is full' do
        expect(overfull_drink.ounces).to(be >= 16)
      end
    end
    ...
    context 'when drink has less than 16 ounces' do
      ...
      subject(:nonfull_drink) { described_class.new('ranja', 12) }

      # remove the 'x' before running this test
      it 'is not full' do
        expect(nonfull_drink.ounces).to(be < 16)
      end
    end
  end
end
